...
public class MainFrame extends JFrame {
	...
	public MainFrame() {
		...
		JButton btnQuery = new JButton(" Indexed ICs");
		btnQuery.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				try {
					if (Lucene.getIndex()!=null) {
						IndexReader reader = DirectoryReader.open(Lucene.getIndex());				
						if (reader.maxDoc()>=0) {
							IndexList indexList = new IndexList(reader);
					...
					JOptionPane.showMessageDialog(null, "Error opening index or index is empty!");
					e.printStackTrace();
				}
			}
		});
		...
		JButton btnImportIcs = new JButton("Insert samples ICs");
		btnImportIcs.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				WaitingDialog waitingDialog = new WaitingDialog( jFrame);
				waitingDialog.setVisible(true);
				if (Lucene.insertAutogeneratedICs()) {
					setEnabled(true);
					waitingDialog.dispose();
					JOptionPane.showMessageDialog(null, Constant.NumberAutogeneratedICs +" ICs generated!");
				} else {
					waitingDialog.dispose();
					setEnabled(true);
					JOptionPane.showMessageDialog(null, "Error while autogenerating ICs!");
				}
			}
		});
